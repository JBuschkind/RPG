...
public class Spieler extends JFrame {
	...
	JTextPane textPaneFightStats = new JTextPane();
	...
	Map<String, String> active_Player;
  ...
  // Die Spalten, die wir mÃ¶chten
  "SELECT stamm.Stamm, klassen.Klasse, beruf.Beruf, ruest_klasse.Ruest, waffen.Waffe "
						
  // Quelltabelle
+ "FROM m_spieler "

  // Tabelle stamm an m_spieler anhÃ¤ngen, dem FremdschlÃ¼ssel entsprechend sortieren
+ "INNER JOIN stamm ON m_spieler.F_Stamm = stamm.ID "

  // klassen an m_spieler anhÃ¤ngen und sortieren
+ "INNER JOIN klassen ON m_spieler.F_Klasse = klassen.ID "

  // beruf an m_spieler anhÃ¤ngen und sortieren
+ "INNER JOIN beruf ON m_spieler.F_Beruf = beruf.ID "


+ "INNER JOIN ruest_klasse ON klassen.F_rüst_klasse = ruest_klasse.ID "
...
+ "INNER JOIN waffen ON l_waffe_klasse.F_Waffe = waffen.ID "
  ...
  // Bedingung: ID des Spielers entspricht der ID, des ausgewÃ¤hlten Spielers
+ "WHERE m_spieler.ID = " + (spielerID.get(0).get("ID")));
				
				ArrayList<Map<String, String>> faehigkeiten = db.queryToMap(
						
  "SELECT faehigkeiten.Fähigkeit FROM m_spieler "
...
//				// Daten auslesen
				...
				for(Map<String, String> element : faehigkeiten){		
					faehigkeitenlist.addElement(element.get("Fähigkeit"));
				}
		...
		btnHinzufgen.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( (!"".equals(txtNewPlayerName.getText())) && (comboNewPlayerStamm.getSelectedIndex() >= 0 ) && (comboNewPlayerBeruf.getSelectedIndex() >= 0) && (comboNewPlayerKlasse.getSelectedIndex() >= 0)){						
					...
					/*TODO
						STÜRTZT AB WENN SPIELERLISTE NEU GELADEN WIRD
					*/
					//LadeSpieler();	
					db.close();
				}
		...
		tabbedPaneInfos.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent ce) {
	            System.out.println("Daten werden aktualisiert ... ");			//DEBUG SPÄTER ENTFERNEN//
	            CharEditorVorbereiten();
	            System.out.println("Character Editor geladen");					//DEBUG SPÄTER ENTFERNEN//
	           // LadeSpieler();	
	            System.out.println("Spielerbögen geladen");						//DEBUG SPÄTER ENTFERNEN//
	            System.out.println("");											//DEBUG SPÄTER ENTFERNEN//
	         }			
		});
		...
		btnEinloggen.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				refreshPlayer();
				PlayerLocation = db.queryToMap("SELECT * FROM `player_location` WHERE `F_m_spieler` = " + active_Player.get("ID"));
				
				if(PlayerLocation.size() < 1){					
					db.execute("INSERT INTO `player_location` (`ID`, `F_m_spieler`, `F_citys`) VALUES (NULL, '" + active_Player.get("ID") + "', '1')");		
					PlayerLocation = db.queryToMap("SELECT * FROM `player_location` WHERE `F_m_spieler` = " + active_Player.get("ID"));
				}
				...
				db.execute("UPDATE `m_spieler` SET `city` = '1', `fight` = '0' WHERE `m_spieler`.`ID` = " + active_Player.get("ID"));
				refreshPlayer();
		...
		panel_5.setLayout(new MigLayout("", "[367.00,grow][130.00,grow]", "[120.00][57.00][grow][]"));
		...
		panel_5.add(panel_18, "cell 0 2,grow");
		panel_18.setLayout(new BorderLayout(0, 0));
		
		
		panel_18.add(textPaneFightStats, BorderLayout.CENTER);
		
		JPanel panel_17 = new JPanel();
		panel_5.add(panel_17, "cell 1 1 1 3,grow");
		...
		panel_5.add(panel_16, "cell 0 3,grow");
		...
		aMapEditorMain.put(enter, new AbstractAction(){
			...
			public void actionPerformed(ActionEvent arg0)	{
				...
				if(active_Player.get("fight").equals("1"))
		...
		aMapEditorMain.put(enter, new AbstractAction(){
			...
			public void actionPerformed(ActionEvent arg0)	{
				...
				if(active_Player.get("city").equals("1")){
				switch (command[0]){				
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//				City						
				case "TravelTo":	ArrayList<Map<String, String>> Reiseziel = db.queryToMap("SELECT * FROM `citys` WHERE `City` LIKE '" + command[1] + "'");
									ArrayList<Map<String, String>> alterStandort = db.queryToMap("SELECT * FROM `player_location` WHERE `F_m_spieler` = " + active_Player.get("ID"));
									...
									refreshPlayer();
										...
										ArrayList<Map<String, String>> enemy_types = db.queryToMap("SELECT * FROM `l_city_enemy_type` WHERE `F_city` = " + alterStandort.get(0).get("ID") + " OR `F_city` = " + Reiseziel.get(0).get("ID"));
										int[] Types = new int[enemy_types.size()];
										for(int i = 0; i < enemy_types.size(); i++){
											Types[i] = Integer.parseInt(enemy_types.get(i).get("F_enemy_Type"));
										}
										prepareFight(Types);
									}
				...
				}else if(active_Player.get("fight").equals("1")){
					textPaneMain.setText("There is a fight going on at the moment");
				}else{
					textPaneMain.setText("There is an ERROR going on, please read the leaflet and ask your nearest Admin");
				}
...
//					fight				
					...
					if(active_Player.get("fight").equals("1")){
						...
						String[] command = editorPaneFight.getText().split("\\s+");
					...
					}else if(active_Player.get("city").equals("1")){						
						textPaneFight.setText("You are not fighting");
					}else{
						textPaneFight.setText("There is an ERROR going on, please read the leaflet and ask your nearest Admin");
					}
	...
	public void LadeSpieler() {
		...
		// Alle Spieler durchgehen und die Namen hinzufÃ¼gen
	...
	public void Setup(){
		...
		LadeSpieler();
		System.out.println("Spielerbögen geladen");	
	...
	public void prepareAttacks(){
		...
		SQLHelper db = VerbindungHerstellen();
		attacks = db.queryToMap("SELECT * FROM `faehigkeiten` WHERE `F_Klasse` = " + active_Player.get("F_Klasse"));
		
		DefaultListModel attack_list = new DefaultListModel();		
		for(Map<String, String> element : attacks){		
			attack_list.addElement(element.get("Fähigkeit"));
		}
		attack_list.addElement("Default_Attack");
	...
	public Integer prepareFight(int[] Types){
			...
			enemy_health =  Integer.parseInt(Enemys.get(gegnerWurf).get("health"));
			textPaneFight.setText("Your Enemy: " + Enemys.get(gegnerWurf).get("Gegner") + "\nHP: " + enemy_health + "\n'" + enemy_voice.get(0).get("Line") + "'");
					
			player_health = Integer.parseInt(active_Player.get("health"));
			player_maxhealth = Integer.parseInt(active_Player.get("max_health"));
			textPaneFightStats.setText("HP: " + player_health + "/" + player_maxhealth + "\n");
			
			prepareAttacks();
			
			
			db.execute("UPDATE `m_spieler` SET `city` = '0', `fight` = '1' WHERE `m_spieler`.`ID` = " + active_Player.get("ID"));
			refreshPlayer();
	...
	public void refreshPlayer(){
		SQLHelper db = VerbindungHerstellen();		
		active_Player = db.queryToMap("SELECT * FROM `m_spieler` WHERE `ID` = " + (daten.get(comboBoxLogin.getSelectedIndex()).get("ID"))).get(0);
		db.close();
	}
}
